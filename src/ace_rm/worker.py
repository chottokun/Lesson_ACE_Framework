...
class BackgroundWorker(threading.Thread):
    ...
    def process_task(self, task: Dict[str, Any]):
        ...
        lang_instruction = "出力は必ず日本語（Japanese）で行ってください。" if config.ACE_LANG == "ja" else "Ensure the output is in English."
        ...
        try:
            res = utils.call_llm_with_retry(self.llm, [HumanMessage(content=prompt)]).content.strip()
            # Basic cleanup
            ...
            if data.get('should_store', False):
                ...
                if similar_docs:
                    ...
                    if existing_doc:
                        ...
                        try:
                            syn_res = utils.call_llm_with_retry(self.llm, [HumanMessage(content=synthesizer_prompt)]).content.strip()
